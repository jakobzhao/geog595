...
In the root directory of geog595, there are folders like assets, labs, and files like readme.md and LICENSE.
...
Once `Visual Studio Code` is opened, press `ctrl+k` and then `ctrl+o` to open the open folder window, navigate to `geog595` folder from your workspace and press `select folder`. Then the `geog595` repo will be opened, and a file tree will be shown in the project list panel on the left of the vscode window.
